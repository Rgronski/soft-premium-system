...
MS-033.0 is COMPLETED / VERIFIED LOCALLY / ACCEPTED as the SPS OS Credit Saving Mode Foundation. SPS OS now documents Credit Saving Mode / Tryb oszczędny as the default economical AI/Codex process: diagnosis before edit, minimal patch, narrow allowed files, explicit forbidden files, targeted tests first, no broad build/dev-server/browser run unless needed, no broad scans, no unrelated refactors, no duplicate usage records, cost classes, model baseline, handoff/report cost fields, and usage-record rules that keep estimated usage separate from real visible credits. `APP_VERSION` is now `1.0080` and `LAST_PUBLISHED_MS` is `MS-033.0 - SPS OS Credit Saving Mode Foundation`; commit and push remain pending.
