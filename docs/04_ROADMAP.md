...
**Purpose**
Show the existing manual Codex flow as one readable process for Product Owner without adding workflow state, persistence, automation, or new behavior.

**Implementation Result**
AI Workspace now shows a compact `Manualny przepływ Codexa` summary in the right Codex panel: `Przygotuj handoff -> Wklej do Codexa -> Wklej raport -> Przejrzyj -> Zaakceptuj -> Opublikuj -> Zwróć raport publikacji`. It states that this is only a process summary and that SPS OS does not add state or automation.

The guidance is read-only copy. It does not add workflow state, checklist persistence, report parsing, automatic validation, automatic report generation, localStorage, Project Brain writes, API calls, Codex automation, execution bridge behavior, cross-project memory lookup, automatic commit/push behavior, or new stateful acceptance/checklist controls.

**Version Gate**
Required and applied locally. `src/lib/app-version.ts` records `APP_VERSION 1.0111` and `LAST_PUBLISHED_MS MS-035.16 - AI Workspace Manual Codex Flow Summary Foundation`.

**Boundary**
MS-035.16 adds no API routes, provider/model configuration, Project Brain storage/write behavior, database/schema files, embeddings/vector search, report parser, automatic validation, persistence, automatic report generation, automatic commit/push behavior, Codex runner, automation, stateful acceptance/checklist controls, Beauty Client PRO repository changes, `C:\SPS_OS_WORK` changes, SPS metadata changes, Project Map canonical changes, checkout-removal behavior, package generator changes, or unrelated AI Workspace generation/save flow changes.

**Verification Evidence**

* `git status -sb` showed local MS-035.16 changes only.
* `git diff --check` passed with standard LF -> CRLF warnings only.
* `npm test -- src/app/projects/[id]/ai/page.test.tsx` passed `47 / 47`.
* `npm test -- src/components/app-version-badge.test.tsx` passed `1 / 1`.
* `git diff --stat` showed `9 files changed, 114 insertions(+), 18 deletions(-)`.
...
**Boundary**
MS-034.1 folds the unpublished MS-034.0 overview-only fix into a wider but still focused lookup sweep. It does not change Project Map behavior, AI workspace behavior, canonical artifacts, Beauty Client PRO files, or broad routing architecture. It was published in Session 100 at commit `d79fe70`.
...
**Type**
SPS OS / Project Map read-only risk review state
...
**Product Outcome**
The Project Map remaining-risk panel now distinguishes accepted, open, and needs-evidence states for SSOT, Project Bible, Project Map, and First Layout. The panel remains read-only and does not treat any state as resolved evidence.
...
**Type**
SPS OS / Project Map Remaining English Copy Cleanup Foundation
...
**Behavior**
* remaining UI labels now use Polish-first wording such as `Aktualny stan`, `Status kandydata`, `Kontekst odłożony`, `Adres repozytorium`, and `Wynik kandydata / evidence`
* common reason and next-step strings are Polish-first, including SSOT review guidance, parked-context empty state, refresh feedback, and evidence drilldown status reasons
* raw terms such as `SSOT`, `candidate`, `evidence`, `map.json`, URLs, IDs, and file paths remain visible where they are data
* copy still states that no `map.json` write happens now
* the action gate remains disabled and non-executing
...
**Implementation Evidence**
* `src/app/projects/[id]/project-map/page.tsx` polishes remaining visible English labels and short reason/next-step strings in the Project Map UI
* `src/app/projects/[id]/project-map/page.test.tsx` verifies Polish labels, removed English UI labels, no-write copy, and disabled action behavior
* `src/lib/app-version.ts` records `APP_VERSION 1.0067` and `LAST_PUBLISHED_MS MS-031.34 - Project Map Remaining English Copy Cleanup Foundation`
* `src/components/app-version-badge.test.tsx` keeps the visible app-version marker aligned
...
**Residual Risk**
* technical domain tokens such as `candidate`, `evidence`, `SSOT`, and file names remain visible by design because they are current Project Map data vocabulary
...
**Type**
SPS OS / Project Map UI Canonical vs Candidate State Foundation
